...
class Decoder(nn.Module):
    ...
    def forward(self, last_pos, last_pos_rel, state_tuple, seq_start_end, x_reference=None):
        """
        Inputs:
        ...
        for i in range(self.seq_len):
            ...
            if (not x_reference == None):
                rel_pos[indices,:] = x_reference[i,:,:2].float()
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, x_reference=None, user_noise=None):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
